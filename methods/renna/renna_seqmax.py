import os
import datetime
import numpy as np
import scipy.io.wavfile
from preprocess import renna_preprocess_wave, check_valid_sequence
from tensorflow.keras.utils import to_categorical # type: ignore
from tensorflow.keras.callbacks import ModelCheckpoint # type: ignore
import tensorflow as tf
import sys
from pyPCG.lr_hsmm import _generate_states
from pyPCG.segment import heart_state, convert_hsmm_states
from pyPCG.io import read_hsannot_file
import scipy.signal as sgn

JOBS = 5
model_name = "rbio3.9_BP_15-100"

SIG_FS = 333
F_FS = 50

# parameters of the neural network and timing
N = 64
tau = N//8
lr = 1e-6

def align_timing(start,end):
    a_start, a_end = start,end
    if start[0] > end[0]:
        a_end = end[1:]
    if start[-1] > end[-1]:
        a_start = start[:-1]
    return a_start, a_end

def seq_max(states):
    re_states = np.copy(states)
    for i in range(1,len(states)):
        if states[i] != (re_states[i-1]+1)%4:
            re_states[i] = re_states[i-1]
    return re_states

if __name__ == "__main__":
    if len(sys.argv)<4:
        print("Please specify training data, testing data and output directories")
        print("<method>.py <train dir> <test dir> <output dir>")
        exit()

    method = sys.argv[0][:-3]
    datapath = sys.argv[1]
    labelpath = datapath
    testpath = sys.argv[2]
    outpath = sys.argv[3]
    if len(sys.argv)>4:
        labelpath = sys.argv[4]

    if not os.path.isdir(outpath):
        os.mkdir(outpath)

    if not os.path.isdir(os.path.join(outpath,method)):
        os.mkdir(os.path.join(outpath,method))

    print("======================================")
    print(f"Training and testing CNN-Seqmax - Processing jobs: {JOBS}")
    print(f"Started: {datetime.datetime.now().time()}")

    print("Reading training data...")

    signals = []
    s1_labels,s2_labels = [],[]
    filenames = []

    for filename in os.listdir(datapath):
        if not filename.endswith("wav"):
            continue
        fs, data = scipy.io.wavfile.read(os.path.join(datapath,filename))
        label = read_hsannot_file(os.path.join(labelpath,f"{filename[:-4]}.csv"))
        filenames.append(filename)
        signal = data - np.mean(data)
        signal = signal / np.max(np.abs(signal))
        signal = sgn.resample_poly(signal,SIG_FS,fs)
        signals.append(signal)
        s1_labels.append(label[0])
        s2_labels.append(label[1])

    signals = np.array(signals,dtype=object)
    s1_labels = np.array(s1_labels,dtype=object)
    s2_labels = np.array(s2_labels,dtype=object)
    filenames = np.array(filenames)

    print("Reading training done!")

    print("Training...")

    model = tf.keras.models.load_model(f"{model_name}.h5") #type:ignore

    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=lr), #type:ignore
                loss=tf.keras.losses.CategoricalCrossentropy(), #type:ignore
                metrics=[tf.keras.metrics.CategoricalAccuracy()]) #type:ignore

    train_data = list()
    train_label = list()
    for sig,s1,s2 in zip(signals,s1_labels,s2_labels):
        states = _generate_states(sig,s1,s2,SIG_FS,F_FS)

        env = renna_preprocess_wave(sig,SIG_FS,F_FS)

        for win in range(0,env.shape[1]-N,tau):
            temp = states[win:(win+N)]-1
            if len(temp)<64:
                break
            train_data.append(env[:,win:(win+N)].T)
            train_label.append(temp)

    train_data = np.array(train_data)
    train_label = np.array(train_label)+1

    train_data, train_label = check_valid_sequence(train_data,train_label,verbose=0)
    train_label = to_categorical(train_label-1)

    callbacks = []
    checkpoint = ModelCheckpoint(f'{model_name}_tuned_seq_complete.h5', monitor='loss', verbose=1, save_best_only=True, mode='min')
    callbacks.append(checkpoint)

    history_finetune = model.fit(train_data,train_label,epochs=15,callbacks=callbacks,verbose=2) #type:ignore

    print("Training done!")
    print("Testing...")
    print("Reading testing data...")
    test_signals = []
    test_fnames = []
    for filename in os.listdir(testpath):
        if not filename.endswith("wav"):
            continue
        fs, data = scipy.io.wavfile.read(os.path.join(testpath,filename))
        test_fnames.append(filename)
        signal = data - np.mean(data)
        signal = signal / np.max(np.abs(signal))
        signal = sgn.resample_poly(signal,SIG_FS,fs)
        test_signals.append(signal)
    print("Reading testing done!")

    s1_pred_out,s2_pred_out,fnames_out = [],[],[]
    test_model = tf.keras.models.load_model(f"{model_name}_tuned_seq_complete.h5") #type:ignore
    for t_signal,t_file in zip(test_signals,test_fnames):
        env = renna_preprocess_wave(t_signal,SIG_FS,F_FS)

        work = list()
        for win in range(0,env.shape[1]-N,tau):
            temp = env[:,win:(win+N)]
            if temp.shape[1]<N:
                break
            work.append(temp.T)
        work = np.array(work)

        pred = test_model.predict(work) #type:ignore

        pred_shape = np.reshape(pred,(-1,4))
        n_windows = pred_shape.shape[0]//N
        s_len = tau*(n_windows-1)+N
        s_expand = np.full((n_windows,s_len,4),np.nan)
        for w in range(n_windows):
            s_expand[w, tau*w:tau*w+N, :] = pred_shape[(w*N):(w*N)+N, :]
        s = np.nanmean(s_expand, axis=0)
        prob = np.squeeze(s)

        state = np.argmax(prob,axis=1)
        states = seq_max(state)+1

        pred_s1s,pred_s1e = convert_hsmm_states(states,state_id=heart_state.S1) #type:ignore
        pred_s2s,pred_s2e = convert_hsmm_states(states,state_id=heart_state.S2) #type:ignore

        a_s1s, a_s1e = align_timing(pred_s1s,pred_s1e)
        a_s2s, a_s2e = align_timing(pred_s2s,pred_s2e)

        pred_s1 = (a_s1s+a_s1e)/2
        pred_s2 = (a_s2s+a_s2e)/2

        s1_pred_out.append(pred_s1/F_FS)
        s2_pred_out.append(pred_s2/F_FS)
        fnames_out.append(t_file)

    print("Testing done!")

    for fname,s1_pred,s2_pred in zip(fnames_out,s1_pred_out,s2_pred_out):
        with open(os.path.join(outpath,method,f"{fname[:-4]}.csv"),"w") as out_file:
            out_file.write("Location;Value\n")
            for s in s1_pred:
                out_file.write(f"{s};S1\n")
            for s in s2_pred:
                out_file.write(f"{s};S2\n")
    print(f"Ended: {datetime.datetime.now().time()}")
    print("======================================")
